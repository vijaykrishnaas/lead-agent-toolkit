...
const MOUNT_PATTERN = /\bapp\.use\(\s*(['"])(.*?)\1\s*,\s*([A-Za-z_$][\w$]*)\s*\)/g;
...
// require()'d from. A mount whose variable has no matching require(...) in the
// same source is skipped (e.g. an inline middleware, not a route module).
